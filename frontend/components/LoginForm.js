...
function LoginForm (props) {
  ...
  async function handleSubmit (e) {
    e.preventDefault()
    const { data: jwt, errors } = await postData('/php/api/login', { email, password });
    // Save data as jwt
    setErrors(errors);
  }
  ...
  return <div className="nes-container with-title">
    ...
    <form onSubmit={handleSubmit}>
      ...
      {errors.length > 0 && <div className="nes-field">
        {errors.map((e, i) => <p className="nes-text is-error" key={i}>{e}</p>)}
      </div>}
